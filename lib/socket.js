...
function on_status_tx(data) {
	switch (data.key.stub) {
		case 'engine':
			console.log('engine.speed          : %s', data.value.full.speed);
			console.log('engine.throttle.pedal : %s', data.value.full.throttle.pedal);
			break;

		case 'lcm':
			console.log('lcm.voltage.terminal_30 : %s', data.value.full.voltage.terminal_30);
			break;

		case 'temperature':
			console.log('temperature.coolant.c : %s', data.value.full.coolant.c);
			break;

		case 'system':
			update.status('client.system.cpu.load_pct', data.value.full.cpu.load_pct);
			update.status('client.system.temperature',  data.value.full.temperature);
			break;

		default:
			json_out(data);
			console.log('UNKNOWN KEY : %s', data.key.full);
	}
}
