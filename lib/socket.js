...
function on_status_tx(data) {
	update.status('client.' + data.key.full, data.value.stub, (data.key.stub !== 'system'));
...
function init(init_cb = null) {
	...
	let uri = config.server.host + ':' + config.server.port + config.server.path;
	let url = 'http://' + uri;

	msg_log('Connecting to ' + uri);
	...
	// let onevent = socket.io.onevent;
	// socket.io.onevent = function (packet) {
	...
	// socket.io.on('*', (event, data) => {
	// 	console.log(event);
	// 	console.log(data);
	// });

	socket.io.on('connect', () => {
		msg_log('Connected to ' + uri);
...
function term(term_cb = null) {
	if (
		...
		typeof status.server.connected === 'undefined' ||
		status.server.connected        === false
	) {
